...
def test_validate_invalid_length(validator, board_2x2, invalid_length_path):
    ...
    assert (
        "3" in result.getErrors[0].getMessage
    )  # Checks if actual length is in the error message
    assert (
        "4" in result.getErrors[0].getMessage
    )  # Checks if expected length is in the error message
