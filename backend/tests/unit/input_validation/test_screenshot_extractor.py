...
def mock_subcomponents():
    ...
    with (
        patch(
            "backend.input_validation.screenshot.screenshot_extractor.ImageLoader"
        ) as mock_img,
        patch(
            "backend.input_validation.screenshot.screenshot_extractor.PaletteDetector"
        ) as mock_pal,
        patch(
            "backend.input_validation.screenshot.screenshot_extractor.GridLocalizer"
        ) as mock_grid,
        patch(
            "backend.input_validation.screenshot.screenshot_extractor.WaypointDetector"
        ) as mock_waypoint,
        patch(
            "backend.input_validation.screenshot.screenshot_extractor.WallDetector"
        ) as mock_wall,
    ):
...
class TestScreenshotExtractor:
    ...
    def setup_method(self):
        ...
        self.sequence_manager.attach_mock(
            self.extractor._image_loader.load_and_preprocess, "load_and_preprocess"
        )
        self.sequence_manager.attach_mock(
            self.extractor._palette_detector.detect_theme, "detect_theme"
        )
        self.sequence_manager.attach_mock(
            self.extractor._grid_localizer.localize_grid, "localize_grid"
        )
        self.sequence_manager.attach_mock(
            self.extractor._waypoint_detector.detect_waypoints, "detect_waypoints"
        )
        self.sequence_manager.attach_mock(
            self.extractor._wall_detector.detect_walls, "detect_walls"
        )
    ...
    def test_happy_path_orchestration_and_schema_formatting(self):
        ...
        expected_calls = [
            ...
            call.detect_waypoints(
                fake_image, fake_bounds, ThemeMode.LIGHT, fake_disc_cells
            ),
        ...
        self.extractor._image_loader.load_and_preprocess.assert_called_once_with(
            payload
        )
        self.extractor._waypoint_detector.detect_waypoints.assert_called_once_with(
            fake_image, fake_bounds, ThemeMode.LIGHT, fake_disc_cells
        )
    ...
    def test_exception_bubbling_waypoint_detection_error(self):
        ...
        self.extractor._waypoint_detector.detect_waypoints.side_effect = (
            WaypointDetectionError("Missing sequence gap")
        )
    ...
    def test_exception_bubbling_image_loader_error(self):
        ...
        self.extractor._image_loader.load_and_preprocess.side_effect = ValueError(
            "Exceeds strict megabyte limit"
        )
