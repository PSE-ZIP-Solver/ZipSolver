...
class AlgorithmicSolver(Solver):
    ...
    def _heuristic(self, current, waypoints, next_wp_idx):
        ...
        if next_wp_idx < len(waypoints):
            ...
            h = abs(current.getX - target_pos.getX) + abs(
                current.getY - target_pos.getY
            )
    ...
    def _parity_ok(self, visited_mask, head, target, board_size, total_cells):
        ...
        if target_colour != (head_colour ^ ((remaining - 1) & 1)):
            return False
        return (
            count[head_colour] == (remaining + 1) // 2
            and count[1 - head_colour] == remaining // 2
        )

    def _is_viable_state(
        self, visited_mask, head, target, board_size, total_cells, adj_list
    ):
        """
        ...
        while queue:
            ...
            for neighbor in adj_list[curr]:
                ...
                if (visited_mask >> n_idx) & 1 == 0 and (
                    reachable_mask >> n_idx
                ) & 1 == 0:
                    reachable_mask |= 1 << n_idx
                    reachable_count += 1
    ...
    def _a_star(self, board):
        ...
        for sp, sidx in seeds:
            ...
            heapq.heappush(pq, SearchNode(-1 + hh, -1, counter, sp, vm, sidx, None))
            counter += 1
        closed_set = set()
        while pq:
            ...
            for n_pos in adj_list[current_node.pos]:
                ...
                if wp:
                    if (
                        n_idx >= len(waypoints)
                        or wp.getOrder != waypoints[n_idx].getOrder
                    ):
                        continue
                    n_idx += 1
                ...
                if (
                    ...
                    and n_pos == target_pos
                    and n_mask != target_mask
                ):
                    continue
                if n_mask != target_mask:
                    if not self._parity_ok(
                        n_mask, n_pos, target_pos, board_size, total_cells
                    ):
                        continue
    ...
    def solve(self, board):
        ...
        if path:
            return SolverResult(
                status=SolverStatus.SOLVED, path=path, message="ok", metrics=m
            )
        if timed_out:
            return SolverResult(
                status=SolverStatus.TIMEOUT, path=None, message="timeout", metrics=m
            )
        return SolverResult(
            status=SolverStatus.UNSOLVABLE, path=None, message="unsolvable", metrics=m
        )
